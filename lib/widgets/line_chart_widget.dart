...
import 'package:mental_monitor/model/data_in_graph.dart';
import 'package:mental_monitor/util.dart';

class LineChartWidget extends StatelessWidget {
  final double minX;
  final double maxX;
  final DataInGraph? dataInGraph;
  LineChartWidget(this.minX, this.maxX, {Map<DateTime, int>? data})
      : dataInGraph = data != null ? DataInGraph(data) : null;
  ...
  Widget build(BuildContext context) {
    return LineChart(
      LineChartData(
        ...
        lineBarsData: [
          LineChartBarData(
            spots: (dataInGraph != null)
                ? dataInGraph!.processData.map((e) {
                    print(e);
                    print((DateTime.now()
                        .dateOnly
                        .difference(e.key)
  ...
  Widget bottomTitleWidgets(double value, TitleMeta meta) {
    ...
    return SideTitleWidget(
      ...
      angle: pi / 2,
      child: Text(
        formatShortBuddhismYear(today
            .subtract(Duration(days: dataInGraph!.dayRange - value.toInt()))),
        style: style,
